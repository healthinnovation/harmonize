...
drones <- read_csv("data/drones.csv") %>% 
  mutate(villages = case_when(
    villages == "12 ABRIL" ~ "12 DE ABRIL",
    villages == "PAUJIL" ~ "EL PAUJIL",
    villages == "VARILLAL" ~"EL VARILLAL",
    TRUE ~ villages
  ))
...
selectInput(
   ...
   choices = c("QUISTOCOCHA","12 DE ABRIL","23 DE FEBRERO","LLANCHAMA","EL PAUJIL","SAN CARLOS",
               "SAN LUCAS", "EL VARILLAL","ZUNGAROCOCHA"),
   multiple = FALSE
   )
```
  ...
  output$map <- renderLeaflet({
    ...
    bbox <- st_read("data/flying_mission_1km2.gpkg") %>% 
      ...
      mutate(
        ...
        name = case_when(
          name == "12 ABRIL" ~ "12 DE ABRIL",
          name == "PAUJIL" ~ "EL PAUJIL",
          name == "EL VARRILLAL" ~ "EL VARILLAL",
          TRUE ~ name)
        ) 
  ...
  observe({
    ...
    if(comunidad == "QUISTOCOHA"){
      ...
      leafletProxy("map") %>%
        ...
        addTiles(urlTemplate = termal_drone, group = sprintf("%s-térmico",comunidad),options = providerTileOptions(maxZoom = 22)) %>% 
        addTiles(urlTemplate = rgb_drone, group = sprintf("%s-ndvi",comunidad),options = providerTileOptions(maxZoom = 22)) %>% 
        addCircleMarkers(
          data = criadero,
      ...
      addLayersControl(
         overlayGroups = c("criadero","cobertura", "focos de criadero","focos de cobertura"),
          baseGroups = c("OpenStreetMap",sprintf("%s-rgb",comunidad), sprintf("%s-térmico",comunidad),sprintf("%s-ndvi",comunidad)),
