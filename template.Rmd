...
```{r}
stations <- read_csv("data/app/stations.csv", col_types = "cciiiDcd")
```

```{r}
diseases <- read_csv("data/app/cases-report.csv") |> rename(
  variable = disease,
  ccpp_name = ccpp)
```
...
```{r out.width=100}
selectInput(
   inputId = "village", label = "Elija una comunidad 🔎", 
   choices = c(
       "Elije uno" = "", "Quistococha", "12 de Abril", "El Paujil", "San Lucas", 
     "El Varillal"
   ),
   multiple = FALSE
)
...
selectInput(
  ...
  choices = c("Día"="Day", "Semana"="Week", "Mes"="Month","Año" ="Year"),
  multiple = FALSE
)
```
...
```{r out.width=100}
sliderInput(
  inputId = "slider_date", label = "Rango de dato 📆",
  min = min(diseases$day),
  max = max(diseases$day),
  value = c(
    min(diseases$day),
    max(diseases$day)
  ), step = 1, ticks = FALSE
...
selectInput(
  inputId = "disease",
  label = "Elija una enfermedad",
  choices = c("Malaria","Dengue","Leptospirosis"),
  multiple = F,
  selected = "Malaria")
```
...
observeEvent(
  input$unit, {
    if (input$unit == "Day") {
      updateSliderInput(
        session, "slider_date", min = min(diseases$day), max = max(diseases$day),
        value = c(min(diseases$day), max(diseases$day)), step = 1
      )
    } else if (input$unit == "Week") {
      updateSliderInput(
        session, "slider_date", min = min(diseases$week), max = max(diseases$week),
        value = c(min(diseases$week), max(diseases$week)), step = 1
      )
    } else if (input$unit == "Month") {
      updateSliderInput(
        session, "slider_date", min = min(diseases$month), max = max(diseases$month),
        value = c(min(diseases$month), max(diseases$month)), step = 1
      )
    } else {
      updateSliderInput(
        session, "slider_date", min = min(diseases$year), max = max(diseases$year),
        value = c(min(diseases$year), max(diseases$year)), step = 1
      )
    }
  }
)
...
<br/> <center><img src="https://raw.githubusercontent.com/healthinnovation/harmonize/main/img/Harmonize_logo.png" width="70%" align="left"/> </center><br/> <br/> <br/> <br/><center><br/><img src="https://raw.githubusercontent.com/healthinnovation/harmonize/app/icons/cayetano.svg" width="70%"/></center>
<br>
<br>
<center>
<button type="button" class="btn btn-primary" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;">
<a href="#" style='color:white;'>Último registro: 07-07-2023</a>
</button>
</center>
...
output$map <- renderLeaflet({
  ...
  observe({
    ...
    if (comunidad == ""){
      ...
      leafletProxy("map") |>
        clearShapes() |>
        addPulseMarkers(data = pto,popup = pto$clasificacion,icon = makePulseIcon(heartbeat = 1,color ='#ff1105')) |> 
...
renderUI({
  if(input$village == ""){
    renderPixfigure({
      pixfigure(portada_url)
    })
  } else {
    plot_unit(
      diseases, vars = input$disease, ccpp = toupper(input$village), 
      unit = input$unit, color = "#8C3A33", title = sprintf("Distribución de casos de %s",input$disease), 
      lower = input$slider_date[1], upper = input$slider_date[2]
    )
  }
})
# "#0B4E60" "#2D819F" "#5AB7B0" "#F5C690" "#8C3A33"
```
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      stations, vars = "rain", ccpp = toupper(input$village), 
...
portada_url <- "https://user-images.githubusercontent.com/36087833/259876753-644217fa-b04a-4fe6-bc69-679ac57dd8f2.jpg"
```
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      stations, vars = "temp_out", ccpp = toupper(input$village), 
      unit = input$unit, color = "#ACD1C5", title = "Temperatura (˚C)", 
      lower = input$slider_date[1], upper = input$slider_date[2] 
    )
  }
})
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      stations, vars = "out_humm", ccpp = toupper(input$village), 
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      stations, vars = "p_2_5_um", ccpp = toupper(input$village), 
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      stations, vars = "p_10_0_um", ccpp = toupper(input$village), 
