---
title: "Meteorological station in the Iquitos - Nauta road 🎏"
output: 
...
```{r out.width=100}
selectInput(
   inputId = "village", label = "Select community 🔎", 
   choices = c(
       "Choose one" = "", "Quistococha", "12 de Abril", "El Paujil", "San Lucas", 
     "El Varillal"
   ),
   multiple = FALSE
)
...
```{r out.width=100}
# selectInput(
#   inputId = "unit", label = "Unit of time ⌚",
...
```{r out.width=100}
# sliderInput(
#   inputId = "slider_date", label = "Date range 📆",
...
sliderInput(
  inputId = "slider_date", label = "Date range 📆",
  min = min(stations$day),
  max = max(stations$day),
  value = c(
    min(stations$day),
    max(stations$day)
  ), step = 1, ticks = FALSE
...
observeEvent(
  input$unit, {
    if (input$unit == "Day") {
      updateSliderInput(
        session, "slider_date", min = min(stations$day), max = max(stations$day),
        value = c(min(stations$day), max(stations$day)), step = 1
      )
    } else if (input$unit == "Week") {
      updateSliderInput(
        session, "slider_date", min = min(stations$week), max = max(stations$week),
        value = c(min(stations$week), max(stations$week)), step = 1
      )
    } else if (input$unit == "Month") {
      updateSliderInput(
        session, "slider_date", min = min(stations$month), max = max(stations$month),
        value = c(min(stations$month), max(stations$month)), step = 1
      )
    } else {
      updateSliderInput(
        session, "slider_date", min = min(stations$year), max = max(stations$year),
        value = c(min(stations$year), max(stations$year)), step = 1
      )
    }
  }
)
...
<p>A web interface that showcases the distribution and data from weather stations and air quality monitors along the Iquitos - Nauta highway axis. The equipment has been installed in 5 communities and collects various data including humidity, temperature, precipitation, as well as PM2.5 and PM10 particles.</p>
...
output$map <- renderLeaflet({
  output$map <- renderLeaflet({
    leaflet() |>
      addProviderTiles(
        provider = providers$Esri.WorldImagery,group = "Esri.WorldImagery"
      ) 
  })
  observe({
    ...
    if (comunidad == ""){
      ...
      leafletProxy("map") |>
        clearShapes() |>
        addPolygons(
          data = region, color = "yellow", fill = NA, weight = 3, opacity = 1
        ) |>
        addMarkers(
          data = pto, 
          icon = iconList(
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      ...
      unit = input$unit, color = "#FF3030", title = "Temperature (˚C)", 
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      ...
      unit = input$unit, color = "#1E90FF", title = "Rain (mm)", 
      lower = input$slider_date[1], upper = input$slider_date[2]
    )
  }
})
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      ...
      unit = input$unit, color = "#87CEEB", title = "Humidity", 
      lower = input$slider_date[1], upper = input$slider_date[2]
    )
  }
})
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      ...
      unit = input$unit, color = "#FF8247", title = "PM2.5", 
      lower = input$slider_date[1], upper = input$slider_date[2]
    )
  }
})
...
renderUI({
  if(input$village == ""){
    ...
    plot_unit(
      ...
      unit = input$unit, color = "#3CB371", title = "PM10", 
      lower = input$slider_date[1], upper = input$slider_date[2]
    )
  }
})
